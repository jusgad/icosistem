...
class URLHandler {
    ...
    build(path, params = {}, options = {}) {
        ...
        try {
            ...
            let resolvedPath = this.resolvePath(path, params);
    ...
    sanitize(url, options = {}) {
        ...
        try {
            let sanitized = url.trim().substring(0, config.maxLength);
...
class EcosystemURLBuilder {
    ...
    seoFriendly(type, data, options = {}) {
        ...
        let path = '';
        let slug = this.generateSlug(data.name || data.title, config.maxLength);
